...
class SRCdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if label_idx == 11 and self.train:
            new_sample_rn = self.draw_silence_sample()
            
            sample = {'audio': [self.draw_silence_sample(), self.draw_silence_sample_mfcc()], 'label': 11}
        else:
            ...
            new_sample_rn = torch.from_numpy(new_sample)
            new_sample_rn = new_sample_rn.type(torch.FloatTensor)
            ...
            if self.mode != "submission":
                sample = {'audio': [new_sample_rn, mfccs], 'label': label_idx}
            else:
                sample = {'audio': [new_sample_rn, mfccs], 'label': item_name}
        return sample
    ...
    def draw_silence_sample_mfcc(self):
        if self.zero_silence < 185: 
            new_sample = np.zeros(seq_length)
            mfccs = self.compute_mfccs(new_sample)
        else:
            selected = self.silence[randint(0, len(self.silence)-1)]
            _, sample = read(self.root_dir+'/_background_noise_/'+selected)
            start_index = randint(0, len(sample)-16000)
...
for i_batch, batch in enumerate(dataloader):
    ...
    print(batch['audio'][0].size())
    print(batch['audio'][1].size())
"""
