...
class SRCdataset(Dataset):
    ...
    def signal_power(self, x):
        signal = x.copy()
        signal = signal / 2**15
        return np.dot(signal.T, signal) / len(signal)
    
    def snr_db(self, signal_p, noise_p):
        return 10 * np.log10(signal_p / noise_p)
    
    def snr(self, signal_p, noise_p):
    ...
    def add_noise(self, sample):
        ...
        if snr_target is None:
            ...
            sample_power = self.signal_power(sample)
            noise_power = self.signal_power(noise)
            snr_current = self.snr_db(sample_power, noise_power)
            factor = np.power(10, (snr_current - snr_target) / 20)
            print(factor)
            return np.int16(sample + factor * noise)
    """
